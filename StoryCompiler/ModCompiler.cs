	...
	class ModCompiler : IDisposable
	{
		...
		public bool Compile(string outputPath, string debugInfoPath, List<string> mods)
		{
			...
			if (mods.Count > 0)
			{
				...
				string userModsFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Larian Studios");
				switch (Game)
				{
					case TargetGame.DOS2:
						userModsFolder = Path.Combine(userModsFolder, "Divinity Original Sin 2", "Mods");
						break;
					case TargetGame.DOS2DE:
						userModsFolder = Path.Combine(userModsFolder, "Divinity Original Sin 2 Definitive Edition", "Mods");
						break;
					case TargetGame.BG3:
						userModsFolder = Path.Combine(userModsFolder, "Baldur's Gate 3", "Mods");
						break;
				}
				...
				{
					Logger.TaskStarted($"Discovering module files at {userModsFolder}");
					visitor.DiscoverUserPackages(userModsFolder);
					Logger.TaskFinished();
				}
				else
				{
					Logger.TaskStarted($"Folder not found at {userModsFolder}");
					Logger.TaskFinished();
				}
