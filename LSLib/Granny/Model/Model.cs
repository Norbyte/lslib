...
namespace LSLib.Granny.Model
{
    ...
    public class Model
    {
        ...
        public node MakeSkeleton()
        {
            ...
            throw new Exception("Model has no root bone!");
        }
    }
        ...
        public static Source FromCollada(source src)
        {
            ...
            if (src.Item is float_array)
            {
                ...
                if ((int)floats.count != floats.Values.Length || floats.count < accessor.stride * accessor.count + accessor.offset)
                    throw new Exception("Source data size mismatch. Check source and accessor item counts.");
            }
            else if (src.Item is Name_array)
            {
                ...
                throw new Exception("Unsupported source data format.");
            ...
            foreach (var param in accessor.param)
            {
                ...
                else if (param.type.ToLower() == "name")
                {
                    ...
                    throw new Exception("Unsupported accessor param type: " + param.type);
    ...
    public class Root
    {
        ...
        private void ImportArtToolInfo(COLLADA collada)
        {
            ...
            ArtToolInfo.RightVector = new float[] { 1, 0, 0 };
            ArtToolInfo.UpVector = new float[] { 0, 1, 0 };
            ArtToolInfo.BackVector = new float[] { 0, 0, -1 };
        ...
        {
            if (skin.source1[0] != '#')
                throw new Exception("Only ID references are supported for skin geometries");

            Mesh mesh = null;
            if (!ColladaGeometries.TryGetValue(skin.source1.Substring(1), out mesh))
                throw new Exception("Skin references nonexistent mesh: " + skin.source1);
            ...
            {
                if (input.source[0] != '#')
                    throw new Exception("Only ID references are supported for joint input sources");

                Source inputSource = null;
                if (!sources.TryGetValue(input.source.Substring(1), out inputSource))
                    throw new Exception("Joint input source does not exist: " + input.source);

                if (input.semantic == "JOINT")
                {
                    List<string> jointNames = inputSource.NameParams.Values.SingleOrDefault();
                    if (jointNames == null)
                        throw new Exception("Joint input source 'JOINT' must contain array of names.");
                    ...
                    foreach (var name in jointNames)
                    {
                        ...
                        if (!skeleton.BonesBySID.TryGetValue(lookupName, out bone))
                            throw new Exception("Joint name list references nonexistent bone: " + lookupName);
                ...
                else if (input.semantic == "INV_BIND_MATRIX")
                {
                    ...
                    if (invBindMatrices == null)
                        throw new Exception("Joint input source 'INV_BIND_MATRIX' must contain a single array of matrices.");
                }
                else
                {
                    throw new Exception("Unsupported joint semantic: " + input.semantic);
                }
            }

            if (joints == null)
                throw new Exception("Required joint input semantic missing: JOINT");

            if (invBindMatrices == null)
                throw new Exception("Required joint input semantic missing: INV_BIND_MATRIX");
            ...
            {
                if (count > 4)
                    throw new Exception("GR2 only supports at most 4 vertex influences");
            }
            ...
            foreach (var input in skin.vertex_weights.input)
            {
                ...
                else if (input.semantic == "WEIGHT")
                {
                    ...
                    if (input.source[0] != '#')
                        throw new Exception("Only ID references are supported for weight input sources");

                    Source inputSource = null;
                    if (!sources.TryGetValue(input.source.Substring(1), out inputSource))
                        throw new Exception("Weight input source does not exist: " + input.source);

                    if (!inputSource.FloatParams.TryGetValue("WEIGHT", out weights))
                        weights = inputSource.FloatParams.Values.SingleOrDefault();

                    if (weights == null)
                        throw new Exception("Weight input source " + input.source + " must have WEIGHT float attribute");
                }
                else
                    throw new Exception("Unsupported skin input semantic: " + input.semantic);
            }

            if (jointInputIndex == -1)
                throw new Exception("Required vertex weight input semantic missing: JOINT");

            if (weightInputIndex == -1)
                throw new Exception("Required vertex weight input semantic missing: WEIGHT");
            ...
            if (boundBones.Count > 255)
                throw new Exception("GR2 supports at most 255 bound bones per mesh.");
